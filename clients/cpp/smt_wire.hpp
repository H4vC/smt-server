...
struct RawSimplifyBlock { std::vector<uint8_t> expression; uint32_t target_node; };
struct RawOptimizationBlock { ScalarValue optimum; bool has_model; std::vector<ModelEntry> model; };
struct RawResponse { uint32_t request_id; uint8_t status, flags; std::vector<uint8_t> payload; };
...
inline RawSimplifyBlock parse_simplify_payload(const std::vector<uint8_t>& payload) { require_bytes(payload, 0, 8, "short simplify"); uint32_t expr_len = read_u32(payload, 0); uint32_t target_node = read_u32(payload, 4); size_t off = 8; require_bytes(payload, off, expr_len, "short simplify expression"); RawSimplifyBlock result; result.target_node = target_node; result.expression.assign(payload.begin() + off, payload.begin() + off + expr_len); off += expr_len; if (off != payload.size()) throw std::invalid_argument("trailing simplify bytes"); return result; }
inline RawOptimizationBlock parse_optimization_payload(const std::vector<uint8_t>& payload, bool has_model) { size_t off = 0; RawOptimizationBlock result{parse_scalar_value(payload, off), has_model, {}}; if (has_model) { std::vector<uint8_t> rest(payload.begin() + off, payload.end()); result.model = parse_model_payload(rest); off = payload.size(); } if (off != payload.size()) throw std::invalid_argument("trailing optimization bytes"); return result; }
...
inline RawResponse parse_response(const std::vector<uint8_t>& data) {
    ...
    RawResponse response{read_u32(data, 4), data[8], data[9], std::vector<uint8_t>(data.begin() + 16, data.end())};
...
public:
    ...
    RawTcpClient(const std::string& host, uint16_t port) { open(host, port); }
    ~RawTcpClient() { close(); }
    RawTcpClient(const RawTcpClient&) = delete;
    RawTcpClient& operator=(const RawTcpClient&) = delete;
    RawTcpClient(RawTcpClient&& other) noexcept : sock_(other.sock_) { other.sock_ = detail::invalid_socket; }
    RawTcpClient& operator=(RawTcpClient&& other) noexcept { if (this != &other) { close(); sock_ = other.sock_; other.sock_ = detail::invalid_socket; } return *this; }

    static RawTcpClient connect(const std::string& host, uint16_t port) { return RawTcpClient(host, port); }
    ...
    RawResponse send_request(const std::vector<uint8_t>& request) { return parse_response(send_payload(request)); }
...
struct ContextState { uint64_t id = next_context_id(); detail::RawBuilder b; };
class Context; class Term; class BVTerm; class BoolTerm;

inline std::string blob_string(const detail::RawBuilder& b, uint64_t payload) {
...
namespace detail { struct RequestContext { std::vector<uint8_t> payload; std::shared_ptr<ContextState> state; std::unordered_map<uint32_t,uint32_t> new_to_old; }; }
...
public:
    ...
    BVTerm bv_and(const BVTerm& a, const BVTerm& b) { return bv_bin(a,b,&detail::RawBuilder::bv_and); } BVTerm bv_or(const BVTerm& a, const BVTerm& b) { return bv_bin(a,b,&detail::RawBuilder::bv_or); } BVTerm bv_xor(const BVTerm& a, const BVTerm& b) { return bv_bin(a,b,&detail::RawBuilder::bv_xor); } BVTerm bv_add(const BVTerm& a, const BVTerm& b) { return bv_bin(a,b,&detail::RawBuilder::bv_add); } BVTerm bv_sub(const BVTerm& a, const BVTerm& b) { return bv_bin(a,b,&detail::RawBuilder::bv_sub); } BVTerm bv_mul(const BVTerm& a, const BVTerm& b) { return bv_bin(a,b,&detail::RawBuilder::bv_mul); }
    ...
    BVTerm bv_udiv(const BVTerm& a, const BVTerm& b) { return bv_bin(a,b,&detail::RawBuilder::bv_udiv); } BVTerm bv_urem(const BVTerm& a, const BVTerm& b) { return bv_bin(a,b,&detail::RawBuilder::bv_urem); } BVTerm bv_sdiv(const BVTerm& a, const BVTerm& b) { return bv_bin(a,b,&detail::RawBuilder::bv_sdiv); } BVTerm bv_srem(const BVTerm& a, const BVTerm& b) { return bv_bin(a,b,&detail::RawBuilder::bv_srem); } BVTerm bv_smod(const BVTerm& a, const BVTerm& b) { return bv_bin(a,b,&detail::RawBuilder::bv_smod); } BVTerm bv_shl(const BVTerm& a, const BVTerm& b) { return bv_bin(a,b,&detail::RawBuilder::bv_shl); } BVTerm bv_lshr(const BVTerm& a, const BVTerm& b) { return bv_bin(a,b,&detail::RawBuilder::bv_lshr); } BVTerm bv_ashr(const BVTerm& a, const BVTerm& b) { return bv_bin(a,b,&detail::RawBuilder::bv_ashr); }
...
private:
    detail::RequestContext build_solve_request(uint32_t request_id, uint32_t budget_ms = 0, bool want_model = true, bool want_core = false) const { return build_request(request_id, detail::command::SOLVE, uint8_t((want_model?detail::request_flags::WANT_MODEL:0)|(want_core?detail::request_flags::WANT_CORE:0)), budget_ms, 0, false); }
    detail::RequestContext build_simplify_request(uint32_t request_id, const Term& target) const { check_same(target); return build_request(request_id, detail::command::SIMPLIFY, 0, 0, target.raw_ref(), true); }
    detail::RequestContext build_minimize_request(uint32_t request_id, const BVTerm& target, bool signed_order=false, uint32_t budget_ms=0, bool want_model=true) const { return build_request(request_id, detail::command::MINIMIZE, uint8_t((signed_order?detail::request_flags::SIGNED:0)|(want_model?detail::request_flags::WANT_MODEL:0)), budget_ms, target.raw_ref(), true); }
    detail::RequestContext build_maximize_request(uint32_t request_id, const BVTerm& target, bool signed_order=false, uint32_t budget_ms=0, bool want_model=true) const { return build_request(request_id, detail::command::MAXIMIZE, uint8_t((signed_order?detail::request_flags::SIGNED:0)|(want_model?detail::request_flags::WANT_MODEL:0)), budget_ms, target.raw_ref(), true); }
...
private:
    ...
    BVTerm bv_bin(const BVTerm& a, const BVTerm& b, uint32_t (detail::RawBuilder::*fn)(uint32_t,uint32_t)) { return bv_term((state_->b.*fn)(expect_bv(a), expect_bv(b))); }
...
class Model { public: bool contains(const Term& t) const { return state_ == t.state_ && values_.count(t.raw_ref()) != 0; } const ScalarValue* get(const Term& t) const { if(state_ != t.state_) return nullptr; auto it=values_.find(t.raw_ref()); return it==values_.end()?nullptr:&it->second; } size_t size() const { return values_.size(); } private: std::shared_ptr<ContextState> state_; std::unordered_map<uint32_t,ScalarValue> values_; friend class Client; };
...
private:
    detail::RawTcpClient transport_; uint32_t next_id_=1;
    Model model_from_entries(const std::vector<detail::ModelEntry>& entries, const detail::RequestContext& req) { Model m; m.state_=req.state; for(const auto& e: entries){ auto it=req.new_to_old.find(e.node_ref); if(it==req.new_to_old.end()) throw protocol_error("unknown model ref"); m.values_[it->second]=e.value; } return m; }
    Response solve_response(const detail::RawResponse& raw, const detail::RequestContext& req) { Response r{}; r.request_id=raw.request_id; r.status=Status(raw.status); r.flags=raw.flags; if(raw.status==detail::status::ERROR || raw.status==detail::status::UNKNOWN){ r.message=std::string(raw.payload.begin(), raw.payload.end()); return r; } if(raw.status==detail::status::SAT){ if(raw.flags & detail::response_flags::HAS_MODEL){ r.model=model_from_entries(detail::parse_model_payload(raw.payload), req); r.has_model=true; } return r; } if(raw.status==detail::status::UNSAT){ if(raw.flags & detail::response_flags::HAS_CORE){ r.core=detail::parse_core_payload(raw.payload); r.has_core=true; } return r; } throw protocol_error("unexpected solve status"); }
    SimplifyResult simplify_response(const detail::RawResponse& raw) { SimplifyResult r; r.request_id=raw.request_id; r.status=Status(raw.status); if(raw.status==detail::status::ERROR || raw.status==detail::status::UNKNOWN){ r.message=std::string(raw.payload.begin(), raw.payload.end()); return r; } if(raw.status!=detail::status::OK) throw protocol_error("unexpected simplify status"); auto block=detail::parse_simplify_payload(raw.payload); r.context=Context::from_expr_bytes(block.expression); r.term=r.context.term_from_ref(block.target_node); r.has_term=true; return r; }
    OptimizationResult optimization_response(const detail::RawResponse& raw, const detail::RequestContext& req) { OptimizationResult r; r.request_id=raw.request_id; r.status=Status(raw.status); r.flags=raw.flags; if(raw.status==detail::status::ERROR || raw.status==detail::status::UNKNOWN){ r.message=std::string(raw.payload.begin(), raw.payload.end()); return r; } if(raw.status==detail::status::UNSAT) return r; if(raw.status!=detail::status::SAT) throw protocol_error("unexpected optimization status"); if((raw.flags & detail::response_flags::HAS_VALUE)==0) throw protocol_error("optimization SAT response missing value"); auto block=detail::parse_optimization_payload(raw.payload, (raw.flags & detail::response_flags::HAS_MODEL) != 0); r.optimum=block.optimum; r.has_value=true; if(block.has_model){ r.model=model_from_entries(block.model, req); r.has_model=true; } return r; }
};
