...
3. `QfbvsmtrsBackend` (`crates/smt-server/src/qfbvsmtrs_backend.rs`) adapts validated wire IR into the standalone `qfbvsmtrs` crate, which owns its own IR, SMT-LIB frontend, bit-blaster, CNF encoder, pure-Rust SAT solver, model extraction, bit-hunt optimization, and deletion-based named unsat-core extraction.
